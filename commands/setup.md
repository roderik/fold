---
description: Add Fold workflow configuration to the current project's CLAUDE.md
allowed-tools: Read, Edit, Write, Glob, AskUserQuestion
---

# Fold Setup

Add the Fold workflow configuration to this project's CLAUDE.md file.

## Steps

1. Check if a `CLAUDE.md` exists in the current working directory. If not, create one.

2. Read the current `CLAUDE.md` content.

3. Check if Fold configuration is already present by looking for the string `Load /caveman` in the file. If found, tell the user Fold is already configured and stop.
...
5. Tell the user: "Fold workflow configured in CLAUDE.md. Run `/teach-impeccable` to set up design context for this project."
